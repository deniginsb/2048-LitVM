...
const { publicClient } = useNetwork();
		...
		try {
					...
					if (tile) {
						...
						if (merged) {
							...
							if (targetTile) {
								...
								newBoardState.tiles = newBoardState.tiles.filter(
									(t) => t.id !== tile.id && t.id !== targetTile.id,
								);
			...
			if (moved) {
				...
				if (moveCount == 3) {
					...
					initializeGameTransaction(activeGameId, boards, moves).catch(
						(error) => {
	...
	const addRandomTile = (boardState: BoardState) => {
		...
		for (let row = 0; row < 4; row++) {
			for (let col = 0; col < 4; col++) {
				if (
					!boardState.tiles.some((tile) => tile.row === row && tile.col === col)
				) {
	...
	const resyncGame = async () => {
		...
		const [latestBoard, nextMoveNumber] =
			await getLatestGameBoard(activeGameId);
	...
	const addRandomTileViaSeed = (
		...
		const seed = hexToBigInt(
			keccak256(
				encodePacked(["bytes32", "uint256"], [gameId, BigInt(moveNumber)]),
			),
		);
	...
	const canMergeWithTile = (
		...
		return (
			targetTile && targetTile.value === tile.value && !targetTile.mergedFrom
		);
	};
